...
由上我们能够分析出来当前所使用微分熵特征提取手段，得到的特征其实是存在大量的冗余，所以这里还可以去尝试其他特征提取方式—如**小波变换**，在频域上进行特征提取等等，还有很多可以尝试的方向。

---

## 尝试2 特征融合

尝试的第二个方向就是去做其他特征的提取并做多种特征融合，根据调研，提取了数据的以下5种特征：

|     特征类型     | 维度 |                  计算方法                  |      物理意义      |
| :--------------: | :--: | :----------------------------------------: | :----------------: |
| **DE (微分熵)**  | 310  |  5频带 × 62通道，每个频带内信号方差的对数  | 频域功率的能量指标 |
| **PSD (功率谱)** | 310  |       Welch方法计算5个频带的平均功率       | 不同频段的信号功率 |
|   **时域特征**   | 248  | 4个统计量 × 62通道：均值、方差、偏度、峰度 | 信号幅度的分布特性 |
|   **频域特征**   | 310  |        5个频带的功率谱集中度等指标         | 频率分布的聚集程度 |
| **高级时域特征** | 186  |        一阶/二阶差分方差、零交叉率         |    信号变化速度    |

**融合总维度**：完整融合（5种）：1364维

直接全部拼接的情况下，两个模型都没有提升。根据之前的经验，对于高维特征来说，SVM不需要进行降维处理，MLP需要进行降维处理。

分别做了以下4次实验：
![subject_accuracy_lines(SVM_Fusion_1364)](https://gitee.com/jak-ma/graph-s/raw/master/imgs/20251203153205164.png)

![subject_accuracy_lines(MLP_Fusion)](https://gitee.com/jak-ma/graph-s/raw/master/imgs/20251203153223610.png)

![subject_accuracy_lines(MLP_Fusion_1364)](https://gitee.com/jak-ma/graph-s/raw/master/imgs/20251203153236001.png)

![subject_accuracy_lines(MLP_Fusion_1364_PCA100)](https://gitee.com/jak-ma/graph-s/raw/master/imgs/20251203153243853.png)

根据以上图显然看出，被试数据的分类难度是不一致的。

同时我也做了其他的非全部的特征融合，比如上面5种特征的其他组合，得到的效果也都差不多分布在 `0.52-0.54`之间。

分析原因：

5种特征都和功率相关，信息冗余度超高；

提取的特征都是静态的，只能拿到一段信号的一个统计值的表现，信号内部的信息直接被丢弃了，比如：90秒信号 → 1个统计值，情绪动态特征可能被忽视；

维度过高，带来更多的噪声，容易误导模型无法正确收敛。
